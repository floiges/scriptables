...
class Widget extends ScriptableWidget {
	...
	constructor(arg) {
		...
		this.logo =
			'https://txc.gtimg.com/data/287371/2020/1105/a8d2e9e19644b244b7a2307bdf2609c0.png';
	...
	async renderBattery(stack, num = 100, size = 'small') {
		...
		// 算法：70/100 * 电量
		const BATTERY_WIDTH = parseInt(
			((SIZE['width'] - SIZE['spacer'] * 2) / 100) * num,
		);
		...
		boxCenter.size = new Size(
			BATTERY_WIDTH,
			SIZE['height'] - SIZE['spacer'] * 2,
		);
		boxCenter.cornerRadius = SIZE['cornerRadius'] / 2;

		boxLeft.addSpacer(
			SIZE['width'] - SIZE['spacer'] * 2 - BATTERY_WIDTH + SIZE['spacer'],
		);
	...
	getPricNum() {
		...
		// 1. 已经过了多少天
		const DAY_TO_NOW = Math.floor(
			(+new Date() - +new Date(this.settings['date'])) / (24 * 60 * 60 * 1000),
		);
		// 2. 百分比
	...
	async renderSmall() {
		...
		const date = w.addText(
			this.settings['name'] +
				' @ ' +
				_date.string(new Date(this.settings['date'])),
		);
