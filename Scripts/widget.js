...
class ScriptableWidget {
	...
	async getWidgetScreenShot(title = null) {
		...
		let phone = PHONE_SIZES[height];
		if (!phone) {
			message =
				'好像您选择的照片不是正确的截图，或者您的机型我们暂时不支持。点击确定前往社区讨论';
...
async function blurImage(img, style) {
	...
	let returnValue = await view.evaluateJavaScript(getBlurJS(blur, style));
