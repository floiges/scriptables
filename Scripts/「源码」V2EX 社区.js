...
class Widget extends ScriptableWidget {
	...
	async renderSmall(data) {
		...
		w = await this.renderHeader(
			w,
			this.logo,
			this.name + ' / ' + this.SETTINGS.split('@')[0],
			Color.white(),
		);
		...
		w.backgroundImage = await this.shadowImage(
			await this.getImageByUrl(
				topic['member']['avatar_large'].replace('mini', 'large'),
			),
		);
	...
	async getDataForTab(tab = 'all') {
		...
		// 解析html
		let tmp = html
			.split(`<div id="Wrapper">`)[1]
			.split(`<div class="inner" style="text-align: right;">`)[0];
		...
		for (let i = 0; i < arr.length; i++) {
			...
			let user = t
				.split(`<a class="node" href="`)[1]
				.split('</strong>')[0]
				.split('<strong>')[1]
				.split('>')[1]
				.split('</')[0];
			let link = t
				.split(`<span class="item_title">`)[1]
				.split('class="')[0]
				.split('"')[1];
			datas.push({
				title,
	...
	async getDataForGo(arg = 'create') {
		...
		let tmp = html
			.split(`<div id="Wrapper">`)[1]
			.split(`<div class="sidebar_units">`)[0];
		let arr = tmp.split(
